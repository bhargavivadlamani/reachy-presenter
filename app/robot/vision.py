    ...
    def __init__(self):
        if _MEDIAPIPE_AVAILABLE:
            self._detector = mp.solutions.face_detection.FaceDetection(
                model_selection=1,          # 1 = full range (up to 5m, better for audiences)
...
def track_faces_during_speech(mini, duration: float) -> None:
    """Track audience faces and move head for natural eye contact.

    Runs at 4 Hz for exactly `duration` seconds — designed to be called
    in a thread alongside audio playback.
    """
    ...
    try:
        while time.time() - start < duration:
            ...
            time.sleep(0.25)  # 4 Hz — matches look_at_image duration
    finally:
        tracker.close()
