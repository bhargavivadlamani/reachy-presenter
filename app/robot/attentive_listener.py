...
_HEAD_DURATION    = 0.4     # head movement duration (s)
_POLL             = 0.25    # loop interval (s)
...
class AttentiveListener:
    ...
    def _look_at_yaw(self, yaw_deg: float, pitch_deg: float = -8.0) -> None:
        try:
            self._mini.goto_target(
                head=create_head_pose(yaw=yaw_deg, pitch=pitch_deg, degrees=True),
                duration=_HEAD_DURATION,
                method="ease_in_out",
            )
    ...
    def _run(self) -> None:
        ...
        while not self._stop.is_set():
            ...
            if active:
                ...
                # Silence — wait a beat then hand control back to scanner
                if scan_paused:
                    if silent_since is None:
                        silent_since = time.time()
                    elif time.time() - silent_since >= _SILENCE_HOLD:
                        self._set_antennas(0.0, 0.0)  # neutral antennas
