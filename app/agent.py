...
_agent = Agent(
    ...
    tools=[present_slide, load_presentation, dance, play_emotion, move_head],
)
...
def run_for_robot(mini: ReachyMini, attention_gate=None) -> None:
    ...
    try:
        asyncio.run(_run_bidi_async(
            ...
            # Longer idle timeout when attention-gated: silences between
            # speakers are expected and should not close the session.
